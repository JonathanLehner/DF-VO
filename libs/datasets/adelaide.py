...
class _3DPW(Dataset):
    ...
    def __init__(self, *args, **kwargs):
        super(_3DPW, self).__init__(*args, **kwargs)
    ...
    def get_intrinsics_param(self):
        ...
        gt_file = f"/home/jonathanlehner/FIFA/GLAMR/datasets/3DPW/sequenceFiles/all/{self.cfg.seq}.pkl"
        with open(gt_file, 'rb') as f:
            data = pickle.load(f, encoding='latin1')
            K = data['cam_intrinsics']
            self.cfg.image.height = int(K[1,2]*2) # J: redundant with below
            self.cfg.image.width = int(K[0,2]*2)

        # each element contains [cx, cy, fx, fy]
        intrinsics_param = [K[0,2], K[1,2], K[0,0], K[1,1]]
        return intrinsics_param
    ...
    def get_image(self, timestamp):
        ...
        try:
            ...
            img_path = os.path.join(self.data_dir['img'], 
                                f"image_{timestamp:05}.{self.cfg.image.ext}")
            print(img_path)
...
class EgoBody(Dataset):
    ...
    def __init__(self, *args, **kwargs):
        super(EgoBody, self).__init__(*args, **kwargs)
    ...
    def get_intrinsics_param(self):
        ...
        gt_data_file = f'../GLAMR/egobody_test_data/egobody_test_gt/{self.cfg.seq}.pkl'

        with open(gt_data_file, 'rb') as f:
            data = pickle.load(f, encoding='latin1')
            K = data['cam_K_gt'][0] # J: camera intrinsics should not change
            self.cfg.image.height = int(K[1,2]*2) # J: redundant with below
            self.cfg.image.width = int(K[0,2]*2)
    ...
    def get_data_dir(self):
        ...
        print(f"img dir is {img_seq_dir}")
        data_dir['img'] = os.path.join(img_seq_dir)
